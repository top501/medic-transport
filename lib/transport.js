...
exports.prototype = {
  ...
  */
  ...
  start: function () {
    ...
    self._drivers.forEach(function(driver) {
      driver.register_receive_handler(function (_message, _callback) {
        ...
        if (_message.from == driver.phone_number) {
          var _err = self._status.validate_status_message(
            _message.content,
            driver.phone_number,
            self._adaptor.default_country_code
            );
          if (_err) {
            self._logger.error(_err);
            return;
          }
    ...
    //fire up the adapter, and tell it to start drivers when it's ready
  ...
  _start_adaptor_with_default_country_code: function(default_country_code, callback) {
    if (!default_country_code) {
    ...
    this._adaptor.default_country_code = default_country_code;
    this._adaptor.start();
    callback();
  },
  ...
  _send_check_status_message: function(driver) {
    ...
    var content = self._status.generate_status_message(
      driver.phone_number, self._adaptor.default_country_code);
    if (!content) {
