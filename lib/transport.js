...
exports.prototype = {
  ...
  */
        ...
        if (_message.from == driver.phone_number &&
          self._validate_status_message(driver, _message.content)) {
            if (this._options.debug) {
              console.log('transport: driver responded successfully to status query');
            }
            _callback(null);
        } else {
  ...
  _check_status: function(driver) {
    if (this._options.debug) {
      process.stderr.write(
        'transport: Verifying driver status - ' + JSON.stringify(driver)
      );
    }
    if (driver.status_pending) {
      console.log('transport: DRIVER DOWN');
      driver.up = false;
    }
  },
  ...
  _validate_status_message: function(driver, message) {
    ...
    if (parts.length != 2) {
      if (this._options.debug) {
        process.stderr.write("transport: message was not status response: " + message + "\n");
      }
      return false;
    }
    ...
    if (period < 0 || period > 60 * 60) {
      if (this._options.debug) {
        process.stderr.write("transport: bad status message timestamp: " + message);
      }
      return false;
    }
    var hash = this._generate_status_message_hash(timestamp, driver);
    if (hash != parts[1]) {
      if (this._options.debug) {
        process.stderr.write("transport: bad status message hash: " + message);
      }
      return false;
    }
    return true;
