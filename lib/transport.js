...
exports.prototype = {
  ...
  STATUS_CHECK_MESSAGE: "CHECK MESSAGE *&^%#",
  ...
  start: function () {
    ...
    self._driver.register_receive_handler(function (_message, _callback) {
      ...
      if (_message.from == self._driver.get_phone_number() &&
        _message.content == self._STATUS_CHECK_MESSAGE) {
  ...
  _send_check_status_message: function() {
    var self = this;
    var status_destination = self._driver.get_phone_number();
    self._driver.send({
      to: status_destination,
      content: self._STATUS_CHECK_MESSAGE
    ...
    setTimeout(self._check_status.bind(this), 60);

    //and schedule the next check
    setTimeout(self._send_check_status_message.bind(this), self._options.status_check_interval * 60);
  },
