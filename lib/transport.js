...
exports.prototype = {
  ...
  start: function () {
    ...
    self._drivers.forEach(function(driver, idx) {
      driver.register_receive_handler(function (_message, _callback) {
        ...
        if (_message.from == self._driver_data[idx].phone_number) {
  ...
  *  Starts the adapter with a given default country code,
  ...
  **/
  _send_check_status_message: function(idx) {
    ...
    var content = self._status.generate_status_message(
      self._driver_data[idx].phone_number, self._adaptor.default_country_code);
    if (!content) {
