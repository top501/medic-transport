  ...
  */
  ...
  _set_processing: function (_req, _res) {
    ...
    _req.body.queued_messages.forEach(function (messageId) {
      ...
      self._logger.debug("moved message from outgoing to waiting", outgoingMessage);
  ...
  _finish_processing: function (_req, _res) {
    if (!_req.body || !_req.body.message_result) {
      this._logger.error("request had invalid body");
      _res.sendStatus(404);
      return;
    }
    var self = this;
    try {
      ...
      this._logger.error("message_result invalid json");
      return;
    }
    ...
    _req.body.message_result.forEach(function (messageResult){
      ...
      for (var messageId in self.waitingMessages) {
        ...
        if (messageResult.sent_result_code == -1 ||
          messageResult.sent_result_code == -2) {
          //it sent! great.
          self._logger.debug("reported successfully sent, " +
            "removed from waiting queue", message);
          message.callback(null, {result: 'success'});
          delete self.waitingMessages[messageId];
          break;
        }
        //save the results to the message
        ...
        //TODO: deal with delivery errors?

        //deal with error
        message.callback(new Error(
          message.sent_result_message
        ), {result: 'failure'});
        delete self.waitingMessages[messageId];
        self._logger.debug("error sending message, " +
          "removed from waiting queue", message);
      }
    });
  ...
  start: function () {
    ...
    var logger = function (req, res, next) {
      self._logger.debug("incoming request", {url: req.url, method: req.method});
