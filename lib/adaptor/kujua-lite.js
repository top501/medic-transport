...
exports.prototype = {
      ...
      if (rc) {
        return self._perform_http_callbacks(rc, 1, function (_e) {
          ...
          if (_e) {
            ...
            rv = { total_sent: 1, status: 'success' }
          }
          ...
          self._invoke_transmit_handler(message, function (_e, _tx_result) {
            ...
            self._debug_print(
              '_handle_transmit_timer: transmit handler ' +
                'returned; transmit status is ' + JSON.stringify(_tx_result)
            );
  ...
  _perform_http_callbacks: function (_callback_object, _depth, _callback) {
    ...
    if (self._debug) {
      self._debug_print(
        '_perform_http_callbacks: ' + JSON.stringify(_callback_object)
      );
    }
    ...
    if (self._debug) {
      self._dump_http_request(request);
    }
    ...
    http_request(request, function (_err, _resp, _body) {
      ...
      if (self._debug) {
        self._dump_http_response(_err, request, _resp, _body);
      }

      /* Conflict */
      if (_resp.statusCode == 201) {
        self._debug_print('HTTP callback caused update conflict; retrying');
  ...
  */
  ...
  _dump_http_request: function (_req) {
    ...
    self._debug_print('_dump_http_request: finished');
   ...
   *   information about the results of an HTTP request.
   */
  _dump_http_response: function (_err, _req, _resp, _body) {

    var self = this;

    self._debug_print('_dump_http_response: starting');
    ...
    self._debug_print('_dump_http_response: finished');
  ...
  _debug_print: function (_string) {
    ...
    return process.stderr.write(_string.replace(/\s+$/, '') + '\n');
  }
