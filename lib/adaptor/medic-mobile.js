...
exports.prototype = {
  ...
  */
  ...
  set_configuration: function(config, callback) {
    var self = this;
    var url = this._url + '/update_settings/medic';
    var body = { 'transport_settings': config }
    self._debug_print('setting configuration: ' + JSON.stringify(body));
    http_request({
      'method': 'PUT',
      'url':url,
      ...
      if (error) {
        self._debug_print('error setting configuration: ' + error);
        return callback(error, false);
      }
      if (response.statusCode != 200) {
        self._debug_print('error setting configuration: bad status code ' + response.statusCode);
        return callback(null, false);
      }
      self._debug_print('set configuration, success: ' + body.success);
  ...
  get_configuration: function (callback) {
    var self = this;
    var url = this._url + '/app_settings/medic';
    http_request({
        url:url,
        json:true,
        auth: { user: this._user, pass: this._pass }
      ...
      if (response.statusCode != 200) {
        self._debug_print('error getting configuration: bad status code ' + response.statusCode);
        return callback(new Error('bad status code ' + response.statusCode), false);
      }
      self._debug_print('successfully retrieved configuration ' +
        JSON.stringify(body.settings.transport_settings));
      return callback(null, body.settings.transport_settings);
    })
  }
};
